...
- [x] **2.3 — La FAQ parle d'un abonnement qui n'existe nulle part** ⚠️ *À nuancer.*
  ...
  **Mais** `QandA.astro:33` répond à « L'abonnement mensuel est obligatoire ? »
  ...
  → **Volet abonnement clos sans attendre le premier client.** Décidé le 30/08
  sur la branche `feat/forfait-suivi-mensuel`, jamais fusionnée ; repris et mis
  en vitrine le 11/09, à la demande du propriétaire : que l'abonnement
  d'hébergement et de maintenance après la création soit écrit clairement sur
  le site. `offreMensuelle` est actif : 25 €/mois ou 250 €/an, démarrage à la
  fin de la première année. La condition posée ici — « activer avec les
  livrables réellement tenus » — n'est donc **pas** remplie : les livrables
  sont dimensionnés par un calcul de marge, pas par une mesure. C'est un pari
  ...
  Le risque de 2.2 — deux tarifs sur la même page — est traité en écrivant la
  règle qui les relie : sous le prix du site (`Offre.astro`) et dans la
  section (`Suivi.astro`), la première année est comprise, donc les deux
  montants ne portent pas sur la même période. **Si l'une des deux mentions
  saute, 2.2 se rouvre.**
...
Le volet abonnement de 2.3 en est sorti : il a été tranché sans la mesure
prévue. Ce qui reste à faire sur ce point n'est plus de l'activer mais de le
vérifier — relever le temps réellement passé chaque mois sur le premier client,
et corriger le contenu du forfait si le budget de quatre heures par an est
dépassé.